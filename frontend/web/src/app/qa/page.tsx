...
export default function QAPage() {
  ...
  const fetchScores = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api.listQAScores({ page, page_size: PAGE_SIZE });
      setItems(data.items);
      setTotal(data.total);
    } catch (err) {
      setError(err instanceof ApiError ? err.debugMessage : err instanceof Error ? err.message : 'Failed to load QA scores');
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  const handleExpand = async (conversationId: string) => {
    ...
    try {
      // Score the conversation to get the detailed breakdown (cached if already scored)
      const data = await api.scoreConversation(conversationId);
      setDetail(data);
    } catch {
      // If scoring fails, just clear
      setDetail(null);
    } finally {
  ...
  return (
    <AppLayout>
      <div className="mx-auto max-w-4xl px-6 py-8">
        ...
        {/* Score button for unscored */}
        <div className="mb-6">
          <Button
            onClick={() => {
              // Score a random unscored conversation — we'll score the first available
              // For demo, use a known conversation pattern
              handleScore('CONV-001');
            }}
            disabled={!!scoringId}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-sm font-medium text-slate-400">No conversations scored yet</p>
            <p className="mt-1 text-xs text-slate-600">
              Click &quot;Score a Conversation&quot; to evaluate quality
            </p>
          </div>
        ) : (
          <>
            <div className="space-y-4">
              {items.map((item) => (
                ...
                >
                  <button
                    onClick={() => handleExpand(item.conversation_id)}
